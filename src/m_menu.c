...
extern int joybescape;
extern int joybyes;
extern int joybchangeweapon;
...
void M_SaveSelect(int choice)
{
  char default_save_name[SAVESTRINGSIZE];
  ...
  snprintf(default_save_name, SAVESTRINGSIZE, "E%dM%d %d", gameepisode, gamemap, I_GetTime());
  strncpy(savegamestrings[choice], default_save_name, SAVESTRINGSIZE);
...
static void M_DrawString(int cx, int cy, int color, const char* ch)
{
  ...
  while (*ch) {
    ...
    // CPhipps - patch drawing updated
    V_DrawNumPatch(cx, cy, 0, hu_font[c].lumpnum, color, VPT_STRETCH | VPT_TRANS, __func__);
...
void M_WriteText (int x,int y,const char* string)
{
  ...
  while(1) {
    ...
    // proff/nicolas 09/20/98 -- changed for hi-res
    // CPhipps - patch drawing updated
    V_DrawNumPatch(cx, cy, 0, hu_font[c].lumpnum, CR_DEFAULT, VPT_STRETCH, __func__);
    cx+=w;
  }
}
