...
int     joybescape;
int     joybyes;
int     joybchangeweapon;

#define JOY_BUTTONS_COUNT 7
...
static int   joyxmove;
static int   joyymove;
static boolean joyarray[JOY_BUTTONS_COUNT + 1];
...
void G_BuildTiccmd(ticcmd_t* cmd)
{
  ...
  if ((!demo_compatibility && players[consoleplayer].attackdown && // killough
       !P_CheckAmmo(&players[consoleplayer])) || gamekeydown[key_weapontoggle] || joybuttons[joybchangeweapon])
...
boolean G_Responder (event_t* ev)
{
  ...
  switch (ev->type)
    ...
    case ev_joystick:
      ...
      joybuttons[1] = ev->data1 & (1 << 1);
      joybuttons[2] = ev->data1 & (1 << 2);
      joybuttons[3] = ev->data1 & (1 << 3);
      joybuttons[4] = ev->data1 & (1 << 4);
      joybuttons[5] = ev->data1 & (1 << 5);
      joybuttons[6] = ev->data1 & (1 << 6);
      joyxmove = ev->data2;
      joyymove = ev->data3;
      return true;    // eat events
