...
boolean I_StartDisplay(void)
{
  ...
  start_displaytime = BREW_GetTicks();
  InDisplay = true;
  return true;
}
...
{
  displaytime = BREW_GetTicks() - start_displaytime;
  InDisplay = false;
}

void I_uSleep(unsigned long usecs)
{
  // Sleeping is no good on BREW.
}
 ...
 */
//extern DoomGenericApp *pApp;

unsigned long I_GetRandomTimeSeed(void)
{
  /* This isnt very random */
  return BREW_GetTicks();
}
 ...
 * on error.
 */
void I_Read(IFile *fd, void* vbuf, size_t sz)
{
  unsigned char* buf = vbuf;

  while (sz) {
    int rc = fread(buf, 1, sz, fd);
    if (rc <= 0) {
...
int I_EnsureSaveDir(const char *path)
{
  return (access(path, R_OK | W_OK) == SUCCESS) ? SUCCESS : mkdir(path, 0);
}
...
char* I_FindFile(const char* wfname, const char* ext)
{
  ...
  //printf("[I_FindFile] search '%s %s'", wfname, ext);
  ...
  for (i = 0; i < sizeof(search)/sizeof(*search); i++) {
    ...
    // printf("[I_FindFile] d='%s'", (d ? d : ""));
    // printf("[I_FindFile] s='%s'", (s ? s : ""));

    sz = (d ? strlen(d) : 0) + (s ? strlen(s) : 0) + pl;

    // printf("[I_FindFile] sz='%d'", sz);

    p = malloc(sz);
    sprintf(p, "%s%s%s%s%s", d ? d : "", (d && !HasTrailingSlash(d)) ? "/" : "",
                             s ? s : "", (s && !HasTrailingSlash(s)) ? "/" : "",
                             wfname);
    //printf("[I_FindFile] p='%s'", p);
