...
void WI_drawAnimatedBack(void)
{
  ...
  for (i=0 ; i<NUMANIMS[wbs->epsd] ; i++)
  {
    ...
    if (a->ctr >= 0)
      // CPhipps - patch drawing updated
      V_DrawNumPatch(a->loc.x, a->loc.y, FB, a->p[a->ctr].lumpnum, CR_DEFAULT, VPT_STRETCH, __func__);
  }
}
...
static int WI_drawNum (int x, int y, int n, int digits)
{
  ...
  while (digits--)
  {
    x -= fontwidth;
    // CPhipps - patch drawing updated
    V_DrawNumPatch(x, y, FB, num[ n % 10 ].lumpnum, CR_DEFAULT, VPT_STRETCH, __func__);
    n /= 10;
  }
