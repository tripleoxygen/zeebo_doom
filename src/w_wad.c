...
// killough 1/31/98: static, const
{
  ...
  if (  strlen(wadfile->name)<=4 || 
  ...
  else
    {
      ...
      BREW_fseek(wadfile->handle, header.infotableofs, _SEEK_START);
...
int (W_CheckNumForName)(const char *name, int li_namespace)
{
  ...
  if(!name)
    printf("W_CheckNumForName name IS NULL");

  // proff 2001/09/07 - check numlumps==0, this happens when called before WAD loaded

  i = ( numlumps == 0 ) ? (-1) : (lumpinfo[W_LumpNameHash(name) % (unsigned)numlumps].index);
...
void W_ReadLump(int lump, void *dest)
{
  ...
  if (lump >= numlumps)
...

    {
      if (l->wadfile)
      {
        fseek(l->wadfile->handle, l->position, _SEEK_START);
