...
{
  lprintf(LO_INFO, "Launch DoomLoop! singletics = %d", singletics);
...
void D_BrewDoomLoop(void)
{
  ...
  BREW_SetTimerCallback(D_BrewDoomLoop);
}
...
void D_AddFile (const char *file, wad_source_t source)
{
  ...
  if (strlen(gwa_filename)>4 && false)
...
static void CheckIWAD(const char *iwadname,GameMode_t *gmode,boolean *hassec)
{
  if ( !access (iwadname,R_OK) )
  {
    ...
    if ((fp = fopen(iwadname, "rb")))
    {
      ...
      // read IWAD header
      if (fread(&header, sizeof(header), 1, fp) == sizeof(header) && !strncmp(header.identification, "IWAD", 4))
      {
        size_t length;
        filelump_t *fileinfo;
        ...
        if (fseek (fp, header.infotableofs, SEEK_SET) ||
            fread (fileinfo, sizeof(filelump_t), length, fp) != (length * sizeof(filelump_t)) ||
            fclose(fp))
...
static void IdentifyVersion (void)
{
  ...
  strcat(basesavegame, "/udata");

  I_EnsureSaveDir(basesavegame);
...
static void D_DoomMainSetup(void)
{
  ...
  //jff 1/22/98 add command line parms to disable sound and music
  {
    ...
    nomusicparm = 1; //nosound || M_CheckParm("-nomusic");
